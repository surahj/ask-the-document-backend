...
Tests all backend API endpoints that the frontend expects to work with
"""
...
# Test database setup
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_frontend.db"
...
class TestDocumentUploadEndpoints:
    ...
    def test_upload_pdf_document(self, client, auth_headers, db_session):
        ...
        with patch(
            ...
            mock_process.return_value = {
                ...
                "file_info": {"file_size": len(pdf_content)},
            }
    ...
    def test_upload_docx_document(self, client, auth_headers):
        ...
        with patch(
            ...
            mock_process.return_value = {
                ...
                "file_info": {"file_size": len(docx_content)},
            }
...
class TestDocumentManagementEndpoints:
    ...
    def test_get_user_documents_pagination(
        ...
        # Test second page
        response = client.get(
            "/api/v1/documents?page=2&page_size=10", headers=auth_headers
        )
        data = response.json()
    ...
    def test_delete_other_user_document(self, client, auth_headers, db_session):
...
class TestChatEndpoints:
    ...
    def test_ask_question_no_documents(self, client, auth_headers):
        """Test asking a question when no documents are uploaded"""
        with patch(
            "app.services.qa_service.QuestionAnsweringService.ask_question"
        ) as mock_qa:
            mock_qa.return_value = {"error": "No documents found for this user"}
    ...
    def test_delete_chat_message(self, client, auth_headers, test_user, db_session):
        ...
        response = client.delete(
            f"/api/v1/chat/messages/{question.id}", headers=auth_headers
        )
        assert response.status_code == 200

        # Verify question is deleted
        deleted_question = db_session.query(Question).filter_by(id=question.id).first()
